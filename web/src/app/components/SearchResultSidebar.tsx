...
export default function SearchResultsSidebar({ events, setEvents }: Props) {
  ...
  const [selectedEventType, setSelectedEventType] = useState<EventTypeOption>(EVENT_TYPE_OPTIONS[0]);
  ...
  // Client-side filtering for search term, date, and event type (optimized)
  const filteredEvents = useMemo(() => {
    let filtered = filteredByTags;

    // Default: show events from now onwards when no date filter is set
    if (!selectedDate) {
      const now = new Date();
    ...
    return filtered;
  }, [filteredByTags, searchTerm, selectedDate, selectedEventType]);
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [searchTerm, selectedDate, selectedTags, selectedEventType]);
      ...
      <div className="flex flex-col gap-2 mb-4 w-full">
        ...
        <div className="flex flex-wrap items-center gap-2">
          {EVENT_TYPE_OPTIONS.map((option) => (
            <button
              key={option.label}
              onClick={() => setSelectedEventType(option)}
