...
from scraper.monitors.academic import (
    DropInScraper,
    OfficeHoursScraper,
    PeerTutoringScraper,
    SIScraper,
...
def scrape_and_export():
                ...
                for event in resource.events:
                    if hasattr(resource, "course_id"):  # Academic event
                        event_data = {
                            ...
                            "Start Time": event.start_datetime.replace(tzinfo=None)
                            if hasattr(event.start_datetime, "replace")
                            else event.start_datetime,
                            "End Time": event.end_datetime.replace(tzinfo=None)
                            if hasattr(event.end_datetime, "replace")
    ...
    if all_events:
        ...
        output_file = os.path.join(
            ...
            f"tartanconnect_events_{timestamp}.xlsx",
        )
