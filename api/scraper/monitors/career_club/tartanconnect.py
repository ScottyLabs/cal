...
class TartanConnectScraper(BaseScraper):
    ...
    def scrape(self):
        ...
        for event in events:
            ...
            categories = (
                event.split('"p22":"')[1]
                .split('"p23')[0]
                .split("List all events filtered by ")[1:]
            )
            ...
            if loc == "-":
                ...
                location = loc
            link = (
                "https://tartanconnect.cmu.edu/rsvp?id="
                + event.split('"p1":"')[1].split('"')[0]
            )
            ...
            if "," in t[1]:
                ...
                start_datetime = datetime.datetime.strptime(
                    t[0].strip() + "M", "%a, %b %d, %Y %I:%M %p"
                )
                end_datetime = datetime.datetime.strptime(
                    t[1].strip(), "%a, %b %d, %Y %I:%M %p"
                )
            else:
                t = t[0] + "M - " + t[1]
                ...
                for i in range(2):
                    if ":" not in times[i]:
                        times[i] = (
                            times[i].split(" ")[0] + ":00 " + times[i].split(" ")[1]
                        )
                start_datetime = datetime.datetime.strptime(
                    date + " " + times[0], "%a, %b %d, %Y %I:%M %p"
                )
                end_datetime = datetime.datetime.strptime(
                    date + " " + times[1], "%a, %b %d, %Y %I:%M %p"
                )
    ...
    def scrape_data_only(self):
        """
        Scrape data and return resources without updating database.
        Used for export-only functionality.
        """
        link = "https://tartanconnect.cmu.edu/mobile_ws/v17/mobile_events_list?range=0&limit=300&filter4_contains=OR&filter4_notcontains=OR&order=undefined&search_word=&&1705612303189"
        s = requests.Session()
