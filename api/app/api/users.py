        ...
        if user:
            ...
            if user.clerk_id != clerk_id:
                ...
                if other and other.id != user.id:
                    other.clerk_id = (
                        None  # or handle a merge if you keep data on 'other'
                    )
                    db.add(other)
                ...
                try:
                    ...
                    return jsonify(
                        {"error": "clerk_id already linked to another user"}
                    ), 409
            ...
            user = get_user_by_clerk_id(db, clerk_id)
            if user is None:
                create_user(db, clerk_id, email=email, fname=fname, lname=lname)
                db.commit()
