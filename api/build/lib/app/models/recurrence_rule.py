...
def parse_by_day_array(
    by_day_list: Optional[List[str]],
) -> Optional[List[Union[weekday]]]:
    """
...
def get_rrule_from_db_rule(rule, event_tz) -> rrule:
    ...
    raw_freq = (
        rule.frequency.value if hasattr(rule.frequency, "value") else rule.frequency
    )
    ...
    if by_month_day:
        kwargs["bymonthday"] = (
            [by_month_day] if isinstance(by_month_day, int) else by_month_day
        )
