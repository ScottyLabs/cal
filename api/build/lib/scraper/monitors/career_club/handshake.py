...
class HandshakeScraper(BaseScraper):
    def __init__(self, db):
        ...
        if not self.auth_token:
            print(
                "Warning: HANDSHAKE_AUTH_TOKEN not set. Please set it to your Handshake authentication token."
            )
            ...
            for event in events:
                ...
                try:
                    start_datetime = datetime.datetime.strptime(
                        node["startDate"], "%Y-%m-%dT%H:%M:%S%z"
                    )
                    end_datetime = datetime.datetime.strptime(
                        node["endDate"], "%Y-%m-%dT%H:%M:%S%z"
                    )
                ...
                if reg_start and reg_end:
                    try:
                        reg_start = datetime.datetime.strptime(
                            reg_start, "%Y-%m-%dT%H:%M:%S%z"
                        )
                        reg_end = datetime.datetime.strptime(
                            reg_end, "%Y-%m-%dT%H:%M:%S%z"
                        )
                    except ValueError:
                ...
                resource = OtherResource(
                    ...
                    metadata={
                        ...
                        "registered_attendees_count": node.get(
