...
def critic_node(state: CourseAgentState):
    ...
    if not website_id or not url or not html:
        print(
            f"No proposed site to critique in critic_node: website_id={website_id}, url={url is not None}, html_length={len(html) if html else 'N/A'}"
        )
        return {
            **state,
            "done": True,
    ...
    if state["verifier_score"] >= 0.9 and state["heuristic_score"] >= 0.7:
        verify_course_website(
            website_id, "Auto-approved (high confidence)", final_score=0.9
        )
        return {
            **state,
    ...
    print(
        f"Critic decision: {decision}, Critic score: {critic_score}, Final score: {final_score}"
    )
    if decision == "accept":
        verify_course_website(
            website_id, f"Approved by CriticAgent; score={final_score:.2f}", final_score
        )
        return {
            **state,
