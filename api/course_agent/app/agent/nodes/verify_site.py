...
from course_agent.app.agent.scores import VERIFIER_SCORES, heuristic_score
from course_agent.app.agent.state import CourseAgentState
from course_agent.app.db.repositories import (
    get_course_website_by_url,
...
def verify_site_node(state: CourseAgentState):
    ...
    if idx >= len(urls):
        return {**state, "done": True, "terminal_status": "no_site_found"}
    ...
    if existing:
        print(
            f"Found existing course website entry for URL {url} with score {existing['relevance_score']}"
        )
